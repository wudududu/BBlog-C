...
(function () {
  ...
  block._tag = '(?!(?:' + 'a|em|strong|small|s|cite|q|dfn|abbr|data|time|code' +
      '|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo' + '|span|br|wbr|ins|del|img)\\b)\\w+(?!:/|@)\\b'
...
(function () {
  ...
  var JAVA_KEYWORDS = [COMMON_KEYWORDS, 'abstract,boolean,byte,extends,final,finally,implements,import,' +
          'instanceof,null,native,package,strictfp,super,synchronized,throws,' + 'transient']
  var CSHARP_KEYWORDS = [JAVA_KEYWORDS, 'as,base,by,checked,decimal,delegate,descending,dynamic,event,' +
  ...
  var JSCRIPT_KEYWORDS = [COMMON_KEYWORDS, 'debugger,eval,export,function,get,null,set,undefined,var,with,' +
          'Infinity,NaN']
  ...
  function combinePrefixPatterns (regexs) {
    ...
    function caseFoldCharset (charSet) {
      var charsetParts = charSet.substring(1, charSet.length - 1).match(new RegExp('\\\\u[0-9A-Fa-f]{4}' +
              '|\\\\x[0-9A-Fa-f]{2}' + '|\\\\[0-3][0-7]{0,2}' + '|\\\\[0-7]{1,2}' + '|\\\\[\\s\\S]' + '|-' +
              '|[^-\\\\]', 'g'))
      var ranges = []
    ...
    function allowAnywhereFoldCaseAndRenumberGroups (regex) {
      var parts = regex.source.match(new RegExp('(?:' + '\\[(?:[^\\x5C\\x5D]|\\\\[\\s\\S])*\\]' +
              '|\\\\u[A-Fa-f0-9]{4}' + '|\\\\x[A-Fa-f0-9]{2}' + '|\\\\[0-9]+' + '|\\\\[^ux0-9]' + '|\\(\\?[:!=]' +
              '|[\\(\\)\\^]' + '|[^\\x5B\\x5C\\(\\)\\^]+' + ')', 'g'))
      var n = parts.length
      var capturedGroups = []
  ...
  function childContentWrapper (element) {
    ...
    for (var c = element.firstChild; c; c = c.nextSibling) {
      var type = c.nodeType
      wrapper = (type === 1) ? (wrapper ? element : c) : (type === 3) ? (notWs.test(c.nodeValue) ? element
        : wrapper) : wrapper
    }
    ...
    var decorate = function (job) {
      ...
      for (var ti = 0, nTokens = tokens.length; ti < nTokens; ++ti) {
        ...
        if (!isEmbedded) {
          ...
          appendDecorations(basePos + tokenStart, token.substring(0, embeddedSourceStart), decorate,
            decorations)
          ...
          appendDecorations(basePos + tokenStart + embeddedSourceEnd, token.substring(embeddedSourceEnd),
  ...
  function sourceDecorator (options) {
    ...
    if (keywords.length) {
      fallthroughStylePatterns.push([PR_KEYWORD, new RegExp('^(?:' + keywords.replace(/[\s,]+/g, '|') + ')\\b'),
        null])
    }
    shortcutStylePatterns.push([PR_PLAIN, /^\s+/, null, ' \r\n\t\xA0'])
    var punctuation = /^.[^\s\w\.$@\'\"\`\/\\]*/
    fallthroughStylePatterns.push([PR_LITERAL, /^@[a-z_$][a-z_$@0-9]*/i, null], [PR_TYPE,
      /^(?:[@_]?[A-Z]+[a-z][A-Za-z_$@0-9]*|\w+_t\b)/, null], [PR_PLAIN, /^[a-z_$][a-z_$@0-9]*/i, null], [
      PR_LITERAL, new RegExp('^(?:' + '0x[a-f0-9]+' + '|(?:\\d(?:_\\d+)*\\d*(?:\\.\\d*)?|\\.\\d\\+)' +
              '(?:e[+\\-]?\\d+)?' + ')' + '[a-z]*', 'i'), null, '0123456789'], [PR_PLAIN, /^\\[\s\S]?/, null], [
  ...
  registerLangHandler(createSimpleLexer([], [[PR_PLAIN, /^[^<?]+/], [PR_DECLARATION, /^<!\w[^>]*(?:>|$)/], [
    PR_COMMENT, /^<\!--[\s\S]*?(?:-\->|$)/], ['lang-', /^<\?([\s\S]+?)(?:\?>|$)/], ['lang-',
    /^<%([\s\S]+?)(?:%>|$)/], [PR_PUNCTUATION, /^(?:<[%?]|[%?]>)/], ['lang-',
  ...
  registerLangHandler(createSimpleLexer([[PR_PLAIN, /^[\s]+/, null, ' \t\r\n'], [PR_ATTRIB_VALUE,
    ...
    PR_ATTRIB_NAME, /^(?!style[\s=]|on)[a-z](?:[\w:-]*\w)?/i], ['lang-uq.val',
    /^=\s*([^>\'\"\s]*(?:[^>\'\"\s\/]|\/(?=\s)))/], [PR_PUNCTUATION, /^[=<>\/]+/], ['lang-js',
    /^on\w+\s*=\s*\"([^\"]+)\"/i], ['lang-js', /^on\w+\s*=\s*\'([^\']+)\'/i], ['lang-js',
    /^on\w+\s*=\s*([^\"\'>\s]+)/i], ['lang-css', /^style\s*=\s*\"([^\"]+)\"/i], ['lang-css',
        ...
        if (prettyPrintRe.test(className) && !prettyPrintedRe.test(className)) {
          ...
          if (!nested) {
            ...
            if (preformattedTagNameRe.test(cs.tagName)) {
              ...
              var whitespace = (currentStyle ? currentStyle['whiteSpace'] : (document.defaultView &&
                              document.defaultView.getComputedStyle) ? document.defaultView.getComputedStyle(cs, null)
